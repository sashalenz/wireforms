...
{{--               value="{{ $value }}"--}}
            ...
            @disabled($disabled)
{{--            {{ $attributes->whereStartsWith(['wire:model', 'wire:change']) }}--}}
        >
        @if($allowClear)
            <span class="absolute inset-y-0 right-0 flex items-center pr-2 cursor-pointer" x-show="$refs.input.value" x-on:click.prevent="$refs.input.flatpickr().clear()">
